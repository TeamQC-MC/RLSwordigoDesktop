...
extern const uint32_t GUEST_MEM_SIZE = 0xC0000000; // 3GB

// Add a specific area for guest-side global variables (libc stuff)
extern const uint32_t GUEST_GLOBALS_BASE = 0x50000; 
...
void start_game_loop(uint32_t env_ptr) {
    ...
    if (updateApp && drawApp) {
        ...
        // Camera override keys
        bool cam_key_left  = false;
        bool cam_key_right = false;
        bool cam_key_fwd   = false;
        bool cam_key_back  = false;
        bool cam_key_up    = false;
        bool cam_key_down  = false;
        bool cam_key_zin   = false;
        bool cam_key_zout  = false;
        ...
        while (running) {
            ...
            // Apply camera position override
                    ...
                    switch (event.type) {
                        ...
                        case SDL_WINDOWEVENT:
                            ...
                            if (event.window.event == SDL_WINDOWEVENT_SIZE_CHANGED) {
                                g_win_w = event.window.data1; g_win_h = event.window.data2;
                            }
                            break;
                        case SDL_MOUSEBUTTONDOWN:
                            if (event.button.button == SDL_BUTTON_LEFT) {
                                mouse_pressed = true;
                                mouse_x = event.button.x; mouse_y = g_win_h - event.button.y;
                                if (gui_visible && g_gui.handle_click(mouse_x, mouse_y)) click_swallowed_by_gui = true;
                                else {
                        ...
                        case SDL_MOUSEMOTION:
                            mouse_x = event.motion.x; mouse_y = g_win_h - event.motion.y;
                        ...
                        case SDL_MOUSEBUTTONUP:
                            ...
                            break;
                        case SDL_KEYDOWN:
                            if (event.key.keysym.sym == SDLK_F1 && !event.key.repeat) { gui_visible = !gui_visible; break; }
                            if (event.key.keysym.sym == SDLK_F2 && !event.key.repeat) { cam_set_active(!g_cam_active); break; }
                            if (event.key.keysym.sym == SDLK_F3 && !event.key.repeat) { debug_visible = !debug_visible; break; }
                            if (event.key.keysym.sym == SDLK_F12 && !event.key.repeat) {
                                ...
                                if (flags & SDL_WINDOW_FULLSCREEN_DESKTOP) {
                                    ...
                                    int fw, fh; SDL_GetWindowSize(g_display_ptr->get_window(), &fw, &fh);
                        ...
                        case SDL_KEYUP: {
                            ...
                            switch (event.key.keysym.sym) {
                                case SDLK_LEFT: case SDLK_a:
                                    if (key_left != is_down) { key_left = is_down; call_handle_touch_event(handleTouchEvent, env_ptr, 0, is_down ? 1 : 2, 6, accumulated_time, 60.0f, 94.0f, 60.0f, 94.0f, is_down ? 1 : 0); } break;
                                case SDLK_RIGHT: case SDLK_d:
                                    if (key_right != is_down) { key_right = is_down; call_handle_touch_event(handleTouchEvent, env_ptr, 0, is_down ? 1 : 2, 7, accumulated_time, 155.0f, 94.0f, 155.0f, 94.0f, is_down ? 1 : 0); } break;
                                case SDLK_SPACE: case SDLK_UP: case SDLK_w:
                                    if (key_jump != is_down) { key_jump = is_down; call_handle_touch_event(handleTouchEvent, env_ptr, 0, is_down ? 1 : 2, 5, accumulated_time, 900.0f, 94.0f, 900.0f, 94.0f, is_down ? 1 : 0); } break;
                                case SDLK_j: case SDLK_z:
                                    if (key_attack != is_down) { key_attack = is_down; call_handle_touch_event(handleTouchEvent, env_ptr, 0, is_down ? 1 : 2, 8, accumulated_time, 790.0f, 94.0f, 790.0f, 94.0f, is_down ? 1 : 0); } break;
                                case SDLK_k: case SDLK_x:
                                    if (key_magic != is_down) { key_magic = is_down; call_handle_touch_event(handleTouchEvent, env_ptr, 0, is_down ? 1 : 2, 10, accumulated_time, 900.0f, 184.0f, 900.0f, 184.0f, is_down ? 1 : 0); } break;
                                case SDLK_i:
                                    if (key_use_item != is_down) { key_use_item = is_down; call_handle_touch_event(handleTouchEvent, env_ptr, 0, is_down ? 1 : 2, 12, accumulated_time, 425.0f, 54.0f, 425.0f, 54.0f, is_down ? 1 : 0); } break;
                                case SDLK_ESCAPE:
                                    if (key_menu != is_down) { key_menu = is_down; call_handle_touch_event(handleTouchEvent, env_ptr, 0, is_down ? 1 : 2, 9, accumulated_time, 48.0f, 500.0f, 48.0f, 500.0f, is_down ? 1 : 0); } break;
                                ...
                                case SDLK_BACKSLASH:     if (is_down) cam_reset(); break;
                            }
                            break;
                        }
                        case SDL_FINGERDOWN: {
                            float x = event.tfinger.x * 960.0f; float y = (1.0f - event.tfinger.y) * 544.0f;
                            int finger_id = (int)(event.tfinger.fingerId % 10) + 20;
                            call_handle_touch_event(handleTouchEvent, env_ptr, 0, 1, finger_id, accumulated_time, x, y, x, y, 1); break;
                        }
                        case SDL_FINGERUP: {
                            float x = event.tfinger.x * 960.0f; float y = (1.0f - event.tfinger.y) * 544.0f;
                            int finger_id = (int)(event.tfinger.fingerId % 10) + 20;
                            call_handle_touch_event(handleTouchEvent, env_ptr, 0, 2, finger_id, accumulated_time, x, y, x, y, 0); break;
                        }
                        case SDL_FINGERMOTION: {
                            ...
                            int finger_id = (int)(event.tfinger.fingerId % 10) + 20;
                            call_handle_touch_event(handleTouchEvent, env_ptr, 0, 4, finger_id, accumulated_time, x, y, old_x, old_y, 0); break;
                        }
                    }
                }
            }
            ...
            if (fps_now - fps_last_time >= 1000) {
                ...
                fps_frame_count = 0; fps_last_time = fps_now;
            }
            completed_frames++;
            ...
            if (completed_frames == 1 || completed_frames == 10 || completed_frames % 100 == 0) {
                std::cout << "[Frame " << completed_frames << "] draws=" << g_frame_stats.draw_calls << " verts=" << g_frame_stats.vertices_submitted << std::endl;
            }
...
void load_and_boot_vanilla() {
    ...
    if (g_main_mod.init_array_vaddr != 0 && g_main_mod.init_array_size > 0) {
        ...
        for (int i = 0; i < g_main_mod.init_array_size/4; i++) {
...
int main(int argc, char* argv[]) {
    ...
    if (!headless) {
        if (display.init(1920, 1080, "Swordigo Desktop - Remastered")) {
            g_display_active = true; g_display_ptr = &display;
        }
    }
    
    init_all();
    
    bool mod_present = (access("libmini.so", F_OK) == 0);
    bool use_mod = false;
    
    if (mod_present) {
        std::cout << "\n[Main] libmini.so detected! Do you want to launch the modded instance? (y/N): ";
        char choice;
        std::cin >> choice;
    ...
    if (use_mod) {
        load_and_boot_modded();
    } else {
        load_and_boot_vanilla();
    }
